...
const APIController = (function() {
    ...
    const clientId = 'ff168e1f64744f11a5ed088b37326037';
    const clientSecret = 'c74a5a01b089440ca493ddaa8027bd32';
    ...
    return {
        ...
        createTrack(id, name, previewUrl) {
            const html = `<a href="#" class="list-group-item list-group-item-action list-group-item-light" id="${id}">
                ${name}
                <button class="btn btn-primary btn-sm play-btn" data-preview-url="${previewUrl}">Play</button>
                <button class="btn btn-danger btn-sm stop-btn" style="display:none;">Stop</button>
            </a>`;
            document.querySelector(DOMElements.divSonglist).insertAdjacentHTML('beforeend', html);
        
            // Show the audio player when a song is clicked
            const audioPlayer = this.inputField().audioPlayer;
...
const APPController = (function(UICtrl, APICtrl) {
    ...
    DOMInputs.tracks.addEventListener('click', async (e) => {
        ...
        if (e.target.classList.contains('play-btn')) {
            // get the preview URL from the data attribute
            const previewUrl = e.target.getAttribute('data-preview-url');
            // play the track
